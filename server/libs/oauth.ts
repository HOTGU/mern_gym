...
const getKakaoToken = async ({
  ...
  const options = {
    code,
    client_id: process.env.KAKAO_OAUTH_CLIENT_ID as string,
    client_secret: process.env.KAKAO_OAUTH_CLIENT_SECRET as string,
    redirect_uri: process.env.KAKAO_OAUTH_REDIRECT_URL as string,
    grant_type: "authorization_code",
  };
  ...
  try {
    ...
    console.log("Failed to fetch Kakao Oauth Tokens");
    throw new Error(err);
  }
};
...
async function getKakaoUser({
  ...
  try {
    const { data } = await axios.get<KakaoUserResult>(
